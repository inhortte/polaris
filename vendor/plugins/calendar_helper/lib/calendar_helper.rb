...
module CalendarHelper
  ...
  def calendar(options = {}, &block)
    ...
    cal << %$<a href="#" class="float-left" onclick="new Ajax.Request('/blog/change_calendar/back', {asynchronous:true, evalScripts:true}); return false;">&lt;-</a>$
    cal << %(<span style="float: left; padding: 0 0 0 10px;">)
    cal << %(<a href=\"/blog/show_date/) + options[:year].to_s + "/" + options[:month].to_s + "\">"
    cal << %(#{Date::MONTHNAMES[options[:month]][0..2]})
    cal << "</a> "
    cal << %(<a href=\"/blog/show_date/) + options[:year].to_s + "\">"
    cal << %(#{options[:year]})
    cal << "</a>"
    cal << %(</span>)
    cal << %$<a href="#" style="float: left; padding: 0 0 0 10px;" onclick="new Ajax.Request('/blog/change_calendar/forward', {asynchronous:true, evalScripts:true}); return false;">-&gt;</a>$
    ...
    first.upto(last) do |cur|
      ...
      if !cell_attrs[:class].nil? && cell_attrs[:class] == 'specialDay'
        cell_text = "<a href=\"/blog/show_date/" + options[:year].to_s + "/" + options[:month].to_s + "/" + cur.mday.to_s + "\">" + cur.mday.to_s + "</a>"
      else
